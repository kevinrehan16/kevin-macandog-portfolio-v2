...
const Experience = () => {
  ...
  return (
    <section id="experience" className="relative py-32 bg-transparent overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div ref={scrollRef} style={{ position: 'relative' }} className="relative">
          ...
          {/* Note: Ang scroll progress line ay natural na nag-a-animate back and forth base sa scroll position */}
          <motion.div
            style={{ scaleY }}
            className="absolute left-0 md:left-1/2 top-0 w-[1px] h-full bg-gradient-to-b from-violet-600 via-fuchsia-500 to-transparent origin-top transform md:-translate-x-1/2"
          />
          ...
          <div className="space-y-20">
            {events.map((e, i) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                // Binago: false at inadjust ang margin para mas swabe ang labas ng cards
              ...
              >
                {/* --- CENTER DOT --- */}
                <div className="absolute left-0 md:left-1/2 w-3 h-3 bg-[#030014] border border-violet-500 rounded-full transform -translate-x-1/2 z-10 shadow-[0_0_15px_rgba(139,92,246,0.5)]" />

                {/* --- CONTENT CARD (GLASSMORPHISM) --- */}
                <div className="w-full md:w-[45%] pl-8 md:pl-0">
                  <div className="group relative p-8 rounded-[2.5rem] bg-white/[0.03] border border-white/10 backdrop-blur-2xl hover:bg-white/[0.05] hover:border-violet-500/50 transition-all duration-700 shadow-2xl">
                    
                    {/* Hover Glow Effect */}
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-[2.5rem] opacity-0 group-hover:opacity-10 blur-xl transition duration-700" />

                    <div className="relative z-10">
                      <span className="text-violet-400 font-mono text-[9px] tracking-[0.3em] font-black uppercase">
                        {e.date}
                      </span>
                      <h3 className="text-2xl font-bold text-white mt-2 group-hover:text-violet-300 transition-colors duration-500">
                        {e.title}
                      </h3>
                      <p className="text-slate-300 font-semibold text-xs mt-1 mb-4 tracking-wide uppercase">
                        {e.company}
                      </p>
